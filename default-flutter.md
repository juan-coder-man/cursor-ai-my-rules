...
- **Always use absolute imports instead of relative imports.**
  - Use `package:project_name/` format for all imports.
  - Avoid using `../` or `./` in import statements.
  - This improves code maintainability and prevents import path issues.
...
- **ALWAYS use predefined themes from themes class**
  - NEVER use Colors.blue, Colors.red, etc. directly in widgets
  - NEVER use TextStyle() directly in widgets
  - ALWAYS retrieve colors at runtime from the current Theme
  - ALWAYS retrieve text styles at runtime from the current Theme
  - This ensures visual consistency and maintainability across the financial app
